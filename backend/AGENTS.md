...
- **Multi-bagger discovery (P1, gated):** `alpha_discovery_enabled` + sub-flags in `config.py`; `build_alpha_score_engine()`, purged walk-forward, class weighting, percentile/DAE scoring. Spec: `docs/multibagger_discovery_engine_v7_cursor_composer_spec.md`. Production Alpha unchanged when discovery is off.
