...
- **Features** (`ml/features.py`): momentum/RS + demand groups (`FUNDAMENTAL/ESTIMATE/SHORT_INTEREST/CATALYST/GRAPH_FEATURE_COLS`), 97 cols total (`demand_feature_columns()`), via `get_feature_columns(include_demand=True, ...)`. **Demand augmenters MUST stay vectorized** (per-ticker `merge_asof` / numpy broadcast) — a per-row loop hangs `build_dataset` for 30–60 min. **Memory (build):** in-place augmenters + `ml/panel_memory.py` (chunked concat, downcast, optional Parquet checkpoint) — ~4.8M rows fit **16 GiB**.
- **Walk-forward** (`ml/xgb_baseline.py`): `slim_dataset_for_training()`, `_walk_forward_frame()`, float32 `_prepare_feature_matrix()` — no full-panel `.copy()` during XGBoost eval. Cloud Run walk-forward needs **32 GiB**; reuse build via `TYCHE_DEMAND_GATE_REUSE_DATASET=true`.
...
- **Training CLI:** `python scripts/train_alpha.py` (`--save-model`, `--feature-set momentum` for peak 62-feature models; `--feature-set demand --sustained` for demand ablation). **Demand gate:** `python scripts/run_demand_gate.py` — builds/reuses `ml/alpha_dataset.parquet`, walk-forward ablation (momentum vs demand) + conditional promotion of `big_move_sustained_*` (97 features; net-new; never overwrites peak). Cloud: **32 GiB** job, reuse build with `TYCHE_DEMAND_GATE_REUSE_DATASET=true`. Verdict → `data/ml/alpha_results/demand_gate_verdict.json`.
...
- **Scheduler:** evening **Mon–Fri** 6 PM + morning **Tue–Sat** 2:30 AM PT (see `deploy_scheduler.sh`); `scheduler_enabled=false` in GCS mode
