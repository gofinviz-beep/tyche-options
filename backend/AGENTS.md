...
- **Features** (`ml/features.py`): momentum/RS + demand groups (`FUNDAMENTAL/ESTIMATE/SHORT_INTEREST/CATALYST/GRAPH_FEATURE_COLS`), 97 cols total (`demand_feature_columns()`), via `get_feature_columns(include_demand=True, ...)`. **Demand augmenters MUST stay vectorized** (per-ticker `merge_asof` / numpy broadcast) — a per-row loop hangs `build_dataset` for 30–60 min. **Memory:** augmenters mutate in-place (no full-panel `.copy()`); `ml/panel_memory.py` downcasts + optional Parquet checkpoint for demand-gate builds (~4.8M rows fit 16 GiB).
